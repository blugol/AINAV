...
const NewsFeed = React.memo(({ isOpen, onClose, newsData, isDarkMode, t, selectedDate, isNewsLoading }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            >
                ...
                {/* Date Header */}
                <div className="px-8 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-gray-50 dark:bg-slate-900/50">
                    <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Showing news for: <span className="font-bold text-gray-900 dark:text-white">{selectedDate}</span>
                    </span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-8 space-y-8 custom-scrollbar">
                    {isNewsLoading ? (
                        <div className="h-full flex flex-col items-center justify-center text-center">
                            <div className="animate-spin text-blue-500 mb-4 h-8 w-8 border-4 border-blue-500 border-t-transparent rounded-full"></div>
                            ...
                            <React.Fragment key={item.id}>
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4">
                                        {item.relatedToolUrl && (item.relatedToolUrl.startsWith('http') || item.relatedToolUrl.startsWith('https')) ? (
                                            <a
                                            ...
                                            <span className="text-xs font-bold text-gray-400 cursor-not-allowed flex items-center gap-1 uppercase tracking-wide">
                                                {item.relatedToolName !== 'AI News' ? 'Try Tool Now' : t.news.visit} <ArrowRight size={12} />
                                            </span>
                                        )}
                                        ...
                                        {item.newsUrl && (item.newsUrl.startsWith('http') || item.newsUrl.startsWith('https')) ? (
                                            ...
                                            <span className="text-xs font-semibold text-gray-400 cursor-not-allowed flex items-center gap-1 uppercase tracking-wide">
                                                {t.news.original} <ExternalLink size={12} />
                                            </span>
                                        )}
                                    </div>
