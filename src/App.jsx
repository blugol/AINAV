...
function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : false; // Default light
  });
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        switch (language) {
          case 'ko':
            rssUrl = 'https://news.google.com/rss/search?q=인공지능+AI+when:1d&hl=ko&gl=KR&ceid=KR:ko';
            break;
          case 'ja':
            rssUrl = 'https://news.google.com/rss/search?q=AI+人工知能+when:1d&hl=ja&gl=JP&ceid=JP:ja';
            break;
          case 'zh':
            // Using Traditional Chinese (Taiwan) or similar accessible feed
            rssUrl = 'https://news.google.com/rss/search?q=人工智能+AI+when:1d&hl=zh-TW&gl=TW&ceid=TW:zh-Hant';
            break;
          default: // 'en'
            rssUrl = 'https://news.google.com/rss/search?q=Artificial+Intelligence+AI+when:1d&hl=en-US&gl=US&ceid=US:en';
            break;
        }
        ...
        if (data.status === 'ok') {
          // Keywords to map news to specific tools
          const TOOL_KEYWORDS = [
          ...
          const formattedNews = data.items.slice(0, 10).map((item, index) => {
            ...
            // Clean up Google News description (often contains HTML table/images)
            let cleanSummary = item.description
              ? item.description.replace(/<[^>]*>?/gm, '').replace(/&nbsp;/g, ' ').substring(0, 150) + '...'
